...
app.post("/api/watched", async (req, res, next) => {
  try {
    ...
    const isWatchlisted = await Watchlist.findOne({
      where: { movie_id: movieId, user_id: userId },
    });
    if (isWatchlisted) {
...
app.post("/api/favorites", async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Movie added to favorite list successfully" });
  } catch (error) {
    console.log("Error:", error);
    next(error);
  }
...
app.get("/api/moviestatus", async (req, res, next) => {
  try {
    ...
    console.log("Decoded:", decoded);
    const userId = decoded.id;
    const movieId = req.query.movieId; // Get movie ID from request body
    console.log("User ID:", userId);
    console.log("Movie ID:", movieId);
